...
const features = [
  {
    ...
    description: "Compose forms with logic, branching, and the feel of a product walkthrough."
  },
  {
    title: "Signal-rich analytics",
    description: "See completion drop-offs, time-to-submit, and friction in a glance."
  },
  {
    title: "Design tokens",
    description: "Ship consistent forms using shared color, type, and motion presets."
  }
];
    ...
    <main className="min-h-screen">
      <header className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 pt-8">
        <div className="flex items-center gap-3">
          ...
          <span className="font-display text-xl tracking-tight">Typeform Studio</span>
        </div>
      ...
      <section className="mx-auto grid w-full max-w-6xl gap-10 px-6 pb-16 pt-12 md:grid-cols-[1.1fr_0.9fr]">
        <div className="space-y-6">
          <p className="text-xs uppercase tracking-[0.3em] text-teal">New release</p>
          ...
          <p className="max-w-xl text-lg text-charcoal/70">
            Launch new ideas faster with a production-ready Next.js foundation tuned for clean design,
            rapid iteration, and measurable outcomes.
          </p>
        ...
        <div className="rounded-3xl border border-charcoal/10 bg-sand p-6 shadow-soft">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="font-display text-2xl text-charcoal">Morning Pulse</p>
            </div>
            <span className="rounded-full bg-ivory px-3 py-1 text-xs text-charcoal/70">Active</span>
          </div>
          <div className="mt-6 space-y-4">
            {metrics.map((metric) => (
              <div key={metric.label} className="rounded-2xl border border-charcoal/10 bg-ivory p-4">
                <p className="text-xs uppercase tracking-[0.2em] text-charcoal/50">
                  {metric.label}
                </p>
                <p className="mt-2 font-display text-2xl text-charcoal">{metric.value}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="mx-auto w-full max-w-6xl px-6 pb-16">
        <div className="flex flex-wrap items-end justify-between gap-6">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-rust">Toolkit</p>
            <h2 className="font-display text-3xl text-charcoal">Everything you need to ship.</h2>
          </div>
          <p className="max-w-md text-sm text-charcoal/70">
            From routing to styling, the stack is wired for clarity. Plug in your product logic and
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-3">
          {features.map((feature) => (
            ...
            >
              <h3 className="font-display text-xl text-charcoal">{feature.title}</h3>
      ...
      <section className="mx-auto w-full max-w-6xl px-6 pb-20">
        <div className="rounded-[32px] border border-charcoal/10 bg-charcoal px-8 py-10 text-ivory md:px-12">
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-ivory/60">Next steps</p>
              <h2 className="mt-2 font-display text-3xl">Turn this into your product.</h2>
              <p className="mt-3 max-w-xl text-sm text-ivory/70">
                Swap the copy, plug in your data sources, and you are ready for production. This
                layout is intentionally minimal so you can move fast.
              </p>
            </div>
